...
var VirtualScroll = (function (_super) {
    ...
    function VirtualScroll(props) {
        ...
        _this.step = _this.props.bufferSize / 3;
        _this.onScrollListener = utils.debounce(_this.onScrollListener.bind(_this), 50);
        return _this;
    }
    ...
    VirtualScroll.prototype.render = function () {
        var _this = this;
        var _a = this.props, list = _a.list, bufferSize = _a.bufferSize;
        var _b = this.state, renderIndex = _b.renderIndex, rowSize = _b.rowSize, hiddenHeight = _b.hiddenHeight;
        var renderList = list.slice(renderIndex, bufferSize + renderIndex);
        console.error(renderList[0].props.id);
        var heightWrapper = ((list.length - renderList.length) * rowSize) - hiddenHeight;
        var style = {
            paddingBottom: heightWrapper + "px",
            paddingTop: hiddenHeight + "px"
        };
        return (React.createElement("div", { ref: function (container) { return _this.container = container; } }, !rowSize ? list[0] :
            React.createElement("div", { style: style, ref: function (wrapper) { return _this.wrapper = wrapper; } }, renderList)));
    };
    VirtualScroll.prototype.onScrollListener = function (event) {
        var rowSize = this.state.rowSize;
        var target = event.target;
        var lastHiddenIndex = Math.floor(target.scrollTop / rowSize);
        // const renderIndex = (lastHiddenIndex - this.step) < 0 ? 0 : (lastHiddenIndex - this.step);
        var hiddenHeight = rowSize * lastHiddenIndex;
    ...
    VirtualScroll.prototype.setHeight = function () {
        var clientRect = this.container.getBoundingClientRect();
        this.setState({
            rowSize: this.container.clientHeight
        });
        this.container.setAttribute('style', "position: fixed; left: " + clientRect.left + "px; top: " + clientRect.top + "px; bottom: 0; right: 0");
        this.setState({
            heightContainer: this.container.clientHeight
        });
        this.container.setAttribute('style', "height: " + this.container.clientHeight + "px");
    };
    VirtualScroll.prototype.calculateHiddenIndex = function () {
        var _this = this;
        var childrenList = Array.from(this.wrapper.children);
        return childrenList.reduce(function (result, item, i) {
            var containerTop = _this.container.getBoundingClientRect().top;
            var itemRect = item.getBoundingClientRect();
            var itemBottom = itemRect.top + itemRect.height;
    ...
    VirtualScroll.prototype.calculateHiddenHeight = function (lastHiddenIndex) {
        var childrenList = Array.from(this.wrapper.children);
        var hiddenChildrenList = childrenList.slice(0, lastHiddenIndex);
        return hiddenChildrenList.reduce(function (result, item) {
